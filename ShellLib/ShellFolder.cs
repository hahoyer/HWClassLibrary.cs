...
namespace ShellLib
{
    public sealed class ShellFolder : IDisposable
    {
        private readonly ICOM _data;

        private ShellFolder(ICOM data)
        {
            _data = data;
        }

        public object Value { get { return _data; } }
        ...
        public static ShellFolder Create(PIDL pidl)
        {
            return new ShellFolder((ICOM)Marshal.GetTypedObjectForIUnknown(pidl.Value, typeof(ICOM)));
        }

        public IntPtr EnumObjects(IntPtr hwnd, ShellApi.SHCONTF flags)
        {
            IntPtr result;
            _data.EnumObjects(hwnd, (int) flags, out result);
            return result;
        }

        public PIDL BindToObject(PIDL pidl, IntPtr pbc, Guid riid)
        {
            Guid riidCopy = riid;
            IntPtr result;
            _data.BindToObject(pidl.Value, pbc, ref riidCopy, out result);
            return PIDL.Create(result);
        }

        public IntPtr BindToStorage(PIDL pidl, IntPtr pbc, Guid riid)
        {
            IntPtr result;
            _data.BindToStorage(pidl.Value, pbc, riid, out result);
            return result;
        }

        public ShellApi.SFGAO GetAttributesOf(ShellApi.SFGAO flags, params PIDL[] apidl)
        {
            UInt32 result = (uint) flags;
            _data.GetAttributesOf((uint)apidl.Length, PIDL.Convert(apidl), ref result);
            return (ShellApi.SFGAO) result;
        }

        public IntPtr GetUIObjectOf(IntPtr hwnd, Guid riid, params PIDL[] apidl)
        {
            UInt32 dummy = default(UInt32);
            IntPtr result;
            _data.GetUIObjectOf(hwnd, (uint)apidl.Length, PIDL.Convert(apidl), riid, ref dummy, out result);
            return result;
        }

        public ShellApi.STRRET GetDisplayNameOf(PIDL pidl, ShellApi.SHGNO flags)
        {
            ShellApi.STRRET result;
            _data.GetDisplayNameOf(pidl.Value, (uint) flags, out result);
            return result;
        }

        public static bool ShouldShow(object psf, IntPtr pidlItem, string[] validExtension)
        {
            return new ShellFolder((ICOM)psf).ShouldShow(PIDL.Create(pidlItem), validExtension);
        }

        private bool ShouldShow(PIDL pidlItem, string[] validExtension)
        {
            // if item is a folder, accept
            if (IsFolder(pidlItem))
                return true;

            // if item is file, check if it has a valid extension
            string sDisplay = NameForParsing(pidlItem).ToUpper();
            for (int i = 0; i < validExtension.Length; i++)
            {
                if (sDisplay.EndsWith("." + validExtension[i].ToUpper()))
                    return true;
            }
        ...
        public bool IsFolder(PIDL pidlItem)
        {
            ShellApi.SFGAO Attrib = GetAttributesOf(ShellApi.SFGAO.SFGAO_FOLDER, pidlItem);
            return Attrib == ShellApi.SFGAO.SFGAO_FOLDER;
        }

        private string NameForParsing(PIDL pidlItem)
        {
            return ConvertToBString(GetDisplayNameOf(pidlItem, ShellApi.SHGNO.SHGDN_FORPARSING),pidlItem);
        }

        private static string ConvertToBString(ShellApi.STRRET name, PIDL pidl)
        {
            return ShellApi.StrRetToBSTR(ref name, pidl);
        }

        public PIDL SHGetRealIDL(PIDL pidl)
        {
            return ShellApi.SHGetRealIDL(this, pidl);
        }
