...
namespace ShellLib
{
    public class ShellApi
    {
        ...
        public static IntPtr SHGetMalloc()
        {
            IntPtr ptrRet;
            ICOM.SHGetMalloc(out ptrRet);
            return ptrRet;
        }

        public static string StrRetToBuf(ref STRRET name, PIDL pidl, int buffsize)
        {
            StringBuilder strDisplay = new StringBuilder(buffsize);
            ICOM.StrRetToBuf(ref name, pidl.Value, strDisplay, (uint) strDisplay.Capacity);
            return strDisplay.ToString();
        }

        public static PIDL SHGetFolderLocation(IntPtr hwnd, CSIDL csidl, IntPtr token)
        {
            IntPtr result;
            ICOM.SHGetFolderLocation(hwnd, (int) csidl, token, 0, out result);
            return PIDL.Create(result);
        }

        public static PIDL SHGetDesktopFolder()
        {
            IntPtr data;
            ICOM.SHGetDesktopFolder(out data);
            return PIDL.Create(data);
        }

        public static string StrRetToBSTR(ref STRRET name, PIDL pidl)
        {
            string display;
            ICOM.StrRetToBSTR(ref name, pidl.Value, out display);
            return display;
        }

        public static PIDL SHParseDisplayName(String path, IntPtr pbc, SFGAO flags, out SFGAO outflags)
        {
            IntPtr result;
            uint oflags;
            ICOM.SHParseDisplayName(path, pbc, out result, (uint) flags, out oflags);
            outflags = (SFGAO) oflags;
            return PIDL.Create(result);
        }

        public static PIDL SHParseDisplayName(String path, IntPtr pbc, ref SFGAO flags)
        {
            return SHParseDisplayName(path, pbc, flags, out flags);
        }

        public static PIDL SHParseDisplayName(String path, IntPtr pbc, SFGAO flags)
        {
            return SHParseDisplayName(path, pbc, flags, out flags);
        }

        public static PIDL SHGetRealIDL(ShellFolder parent, PIDL pidl)
        {
            IntPtr result;
            IShellFolder psf = (IShellFolder) parent.Value;
            ICOM.SHGetRealIDL(psf, pidl.Value, out result);
            return PIDL.Create(result);
        }

        public static string SHGetPathFromIDList(PIDL pidl)
        {
            StringBuilder result = new StringBuilder(1000);
            ICOM.SHGetPathFromIDList(pidl.Value, result);
        ...
        private static class ICOM
        {
            ...
            public static extern Int32 SHParseDisplayName(
                ...
                // Pointer to a zero-terminated wide string that contains the display name 
                // to parse. 
        ...
        public enum SFGAO : uint
        {
            ...
            SFGAO_ALL = 0x70CFC17F // for determining storage capabilities, ie for open/save semantics
        }
