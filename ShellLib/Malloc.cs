...
namespace ShellLib
{
    internal sealed class Malloc : IDisposable
    {
        [ComImport]
        ...
        static private Malloc _instance;

        readonly ICOM _data;

        internal Malloc()
        {
            _data = (ICOM)Marshal.GetTypedObjectForIUnknown(ShellApi.SHGetMalloc(), typeof(ICOM));
        }

        internal static Malloc Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new Malloc();
                return _instance;
            }
        }
        ...
        internal void Free(IntPtr value)
        {
            _data.Free(value);
        }
    }
