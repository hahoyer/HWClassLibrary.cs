...
public sealed class SourcePosition : Dumpable, IEquatable<SourcePosition>
{
    ...
    /// </summary>
    public char Current => Source[new Index64(Position)];

    /// <summary>
    ///     Natural indexer
    /// </summary>
    public char this[int index] => Source[new Index64(Position + index)];
    ...
    public long LineIndex => Source.LineIndex(Position);
    ...
    public string this[Range64 range]
        => Source[
            new Range64(Position + range.Start.GetOffset(Source.Length), Position + range.End.GetOffset(Source.Length))];
    ...
    public SourcePosition(Source source, long position)
    {
        Position = position;
        Source = source;
    }
    ...
    /// <param name="source"></param>
    /// <param name="position"></param>
    public SourcePosition(Source source, Index64 position)
    ...
    public static SourcePosition operator +(SourcePosition target, long y)
        => target.Source + (target.Position + y);

    public static SourcePosition operator -(SourcePosition target, long y)
        => target.Source + (target.Position - y);

    public static long operator -(SourcePosition target, SourcePosition y)
    {
    ...
    public long? Match(IMatch automaton, bool isForward = true)
    {
    ...
    public bool StartsWith(string data, StringComparison type = StringComparison.InvariantCulture)
        => this[new Range64(0,data.Length)].Equals(data, type);

    public SourcePart Span(SourcePosition other) => SourcePart.Span(new(Source, Position), other);
    public SourcePart Span(long length) => SourcePart.Span(new(Source, Position), length);
}